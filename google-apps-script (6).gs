...
// TEST COMPLETO — ejecuta esto y revisa los logs
function testCompleto() {
  ...
  try {
    ...
    Logger.log('❌ ERROR Sheet: ' + e.toString());
    return;
  }

  // 2. Probar crear hoja Datos
  try {
    var ws = ss.getSheetByName('Datos');
    if (ws) {
      Logger.log('✅ Hoja Datos existe con ' + ws.getLastRow() + ' filas');
    } else {
      Logger.log('⚠️ Hoja Datos no existe - se creara al grabar');
    }
  } catch(e) {
    Logger.log('❌ ERROR hoja: ' + e.toString());
    return;
  }

  // 3. Probar funcion coord()
  try {
    var c = coord('18.486100');
    Logger.log('✅ coord() OK: ' + c);
  } catch(e) {
    Logger.log('❌ ERROR coord(): ' + e.toString());
    return;
  }

  // 4. Probar funcion linkMaps()
  try {
    var m = linkMaps('18.486100', '-69.931200');
    Logger.log('✅ linkMaps() OK: ' + m);
  } catch(e) {
    Logger.log('❌ ERROR linkMaps(): ' + e.toString());
    return;
  }

  // 5. Simular guardado completo de Datos
  try {
    var testData = {
      ...
      lat: '18.486100',
      lng: '-69.931200',
      tipo_letrero: 'Letras Encajonadas',
      caracteristica: '1 Cara',
      cantidad: '2',
      medida: '10 ft2',
      poligono: 'III',
      observacion: 'Prueba',
    ...
    guardarFila('Datos', testData, '');
    Logger.log('✅ guardarFila() OK - revisa la hoja Datos');
  } catch(e) {
    Logger.log('❌ ERROR guardarFila(): ' + e.toString());
    return;
  }

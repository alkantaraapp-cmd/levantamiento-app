...
function agregarNicho() {
  ...
  // Build HTML using DOM to avoid quote issues
  ...
  div.innerHTML = [
    '<div class="nicho-card-header">',
      ...
      '</div>',
      '<button class="btn-remove-nicho" onclick="eliminarNicho(' + idx + ')">✕ Eliminar</button>',
    '</div>',
    ...
    '<div class="nicho-fields">',
      ...
      '<div class="field-group full"><label>Estado del Nicho *</label>',
        '<div class="toggle-group-wrap" id="nicho-estado-grp-' + idx + '">',
          '<button class="toggle-btn-sm" id="nicho-estado-oc-' + idx + '">OCUPADO</button>',
          '<button class="toggle-btn-sm" id="nicho-estado-va-' + idx + '">VACÍO</button>',
        '</div>',
      '</div>',

      // Terminal
      '<div class="field-group full"><label>Terminal del Nicho *</label>',
        '<div class="toggle-group-wrap" id="nicho-terminal-grp-' + idx + '">',
      ...
      '<div class="field-group"><label>Nombre del Difunto</label>',
        '<input type="text" id="nicho-nombre-' + idx + '" placeholder="Nombre (vacío si nicho vacío)">',
      '</div>',
  ...
  // Attach events via JS (avoids all inline quote issues)
  // Estado buttons
  div.querySelector('#nicho-estado-oc-' + idx).addEventListener('click', function() { setNichoEstado(idx, this, 'OCUPADO'); });
  div.querySelector('#nicho-estado-va-' + idx).addEventListener('click', function() { setNichoEstado(idx, this, 'VACÍO'); });

  // Terminal buttons
  terminales.forEach(function(l) {
    div.querySelector('#nicho-term-' + idx + '-' + l).addEventListener('click', function() { setNichoTerminal(idx, this, l); });
  });

  // Posicion select
  div.querySelector('#nicho-posicion-' + idx).addEventListener('change', function() { setNichoPosicion(idx, this.value); });

  // Nombre input
  div.querySelector('#nicho-nombre-' + idx).addEventListener('input', function() { setNichoNombre(idx, this.value); });

  // Fecha input
  div.querySelector('#nicho-fecha-' + idx).addEventListener('change', function() { setNichoFecha(idx, this.value); });

  // Edad input
  div.querySelector('#nicho-edad-' + idx).addEventListener('input', function() { setNichoEdad(idx, this.value); });

  // Obs textarea
  div.querySelector('#nicho-obs-' + idx).addEventListener('input', function() { setNichoObs(idx, this.value); });

  // Camera button
  div.querySelector('#nicho-cam-btn-' + idx).addEventListener('click', function() {
    div.querySelector('#nicho-cam-' + idx).click();
  });
  div.querySelector('#nicho-cam-' + idx).addEventListener('change', function() { handleNichoPhoto(idx, this); });

  // Gallery button
  div.querySelector('#nicho-gal-btn-' + idx).addEventListener('click', function() {
    div.querySelector('#nicho-gal-' + idx).click();
  });
  div.querySelector('#nicho-gal-' + idx).addEventListener('change', function() { handleNichoPhoto(idx, this); });
